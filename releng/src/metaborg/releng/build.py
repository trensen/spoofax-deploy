...
def BuildAll(repo, components=['all'], buildDeps=True, resumeFrom=None, buildStratego=False,
  ...
  for build in buildOrder:
    ...
    result = cmd(basedir=basedir, deploy=deploy, release=release, qualifier=qualifier, noSnapshotUpdates=True,
      clean=clean, profiles=list(profiles), buildStratego=buildStratego, bootstrapStratego=bootstrapStratego,
      strategoTest=strategoTest, skipExpensive=skipExpensive, skipTests=skipTests, resumeFrom=resumeFrom,
      localRepo=localRepo, **mavenArgs)
    if result:
...
def BuildPoms(basedir, deploy, qualifier, buildStratego, bootstrapStratego, strategoTest, skipExpensive, **kwargs):
  ...
  cwd = path.join(basedir, 'releng', 'build', 'parent')
...
def BuildPremadeJars(basedir, deploy, release, clean, qualifier, buildStratego, bootstrapStratego,
  ...
  newExtraArgs = ' -DpomFile="{}" -Dfile="{}" -DrepositoryId={} -Durl={}'.format(makePermissivePom, makePermissiveJar,
    repositoryId, deployUrl)
  if extraArgs:
    extraArgs += newExtraArgs
  else:
    extraArgs = newExtraArgs

  Mvn(cwd=cwd, clean=False, phase=phase, extraArgs=extraArgs, **kwargs)
...
def DownloadStrategoXt(basedir, clean, profiles, skipExpensive, **kwargs):
  ...
  cwd = path.join(basedir, 'strategoxt', 'strategoxt')
  Mvn(cwd=cwd, pomFile='download-pom.xml', clean=False, profiles=buildProfiles, phase='dependency:resolve', **kwargs)
  return BuildResult([])
...
def BuildStrategoXt(basedir, deploy, bootstrap, runTests, skipTests, skipExpensive, **kwargs):
  ...
  if bootstrap:
    pomFile = path.join('bootstrap-pom.xml')
  else:
    pomFile = path.join('build-pom.xml')
  ...
  # Build StrategoXT
  Mvn(cwd=strategoXtDir, pomFile=pomFile, phase=phase, skipTests=skipTests, **buildKwargs)

  # Build StrategoXT parent POM
  buildKwargs = dict(kwargs)
  buildKwargs.update({'strategoxt-skip-build': True, 'strategoxt-skip-assembly': True})

  parentPomFile = path.join('buildpoms', 'pom.xml')
  Mvn(cwd=strategoXtDir, pomFile=parentPomFile, phase=phase, skipTests=skipTests, **buildKwargs)

  if bootstrap:
    distribDir = path.join(strategoXtDir, 'buildpoms', 'bootstrap3', 'target')
  else:
    distribDir = path.join(strategoXtDir, 'buildpoms', 'build', 'target')

  return BuildResult([
    BuildArtifact('StrategoXT distribution', glob('{}/strategoxt-distrib-*-bin.tar'.format(distribDir))[0],
      'strategoxt-distrib.tar'),
    BuildArtifact('StrategoXT JAR', '{}/dist/share/strategoxt/strategoxt/strategoxt.jar'.format(distribDir),
      'strategoxt.jar'),
    BuildArtifact('StrategoXT minified JAR',
      glob('{}/buildpoms/minjar/target/strategoxt-min-jar-*.jar'.format(strategoXtDir))[0],
      'strategoxt-min.jar'),
  ])
...
def BuildJava(basedir, qualifier, deploy, buildStratego, bootstrapStratego, strategoTest, skipExpensive, **kwargs):
  ...
  cwd = path.join(basedir, 'releng', 'build', 'java')
  Mvn(cwd=cwd, phase=phase, forceContextQualifier=qualifier, **kwargs)

  return BuildResult([
    BuildArtifact('Spoofax sunshine JAR', glob(
      path.join(basedir, 'spoofax-sunshine/org.metaborg.sunshine2/target/org.metaborg.sunshine2-*.jar'))[0],
...
def BuildLanguagePoms(basedir, deploy, **kwargs):
  ...
  cwd = path.join(basedir, 'releng', 'build', 'language', 'parent')
  Mvn(cwd=cwd, phase=phase, **kwargs)
...
def BuildLanguages(basedir, deploy, qualifier, buildStratego, bootstrapStratego, strategoTest, skipExpensive, **kwargs):
  ...
  cwd = path.join(basedir, 'releng', 'build', 'language')
  Mvn(cwd=cwd, phase=phase, **kwargs)
...
def BuildSPT(basedir, deploy, qualifier, buildStratego, bootstrapStratego, strategoTest, skipExpensive, **kwargs):
  ...
  cwd = path.join(basedir, 'releng', 'build', 'language', 'spt')
  Mvn(cwd=cwd, phase=phase, **kwargs)
...
def BuildEclipseDeps(basedir, qualifier, deploy, buildStratego, bootstrapStratego, strategoTest, skipExpensive,
    **kwargs):
  phase = 'deploy' if deploy else 'install'

  cwd = path.join(basedir, 'releng', 'build', 'eclipse', 'deps')
  Mvn(cwd=cwd, phase=phase, forceContextQualifier=qualifier, **kwargs)
...
def BuildEclipse(basedir, qualifier, deploy, buildStratego, bootstrapStratego, strategoTest, skipExpensive, **kwargs):
  ...
  cwd = path.join(basedir, 'releng', 'build', 'eclipse')
  Mvn(cwd=cwd, phase=phase, forceContextQualifier=qualifier, **kwargs)

  return BuildResult([
    BuildArtifact('Spoofax Eclipse update site', path.join(basedir,
...
def BuildSpoofaxLibs(basedir, deploy, qualifier, buildStratego, bootstrapStratego, strategoTest, skipExpensive,
  ...
  return BuildResult([
    BuildArtifact('Spoofax libraries JAR', glob(path.join(basedir, 'releng/build/libs/target/build.libs-*.jar'))[0],
      'spoofax-libs.jar'),
  ])
...
def GenerateMavenSettings(location=_mvnSettingsLocation, metaborgReleases=_metaborgReleases,
    metaborgSnapshots=_metaborgSnapshots, spoofaxUpdateSite=_spoofaxUpdateSite,
    centralMirror=_centralMirror):
  repositories = []
  if metaborgReleases:
    repositories.append(
